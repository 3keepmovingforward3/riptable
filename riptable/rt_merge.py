...
def _build_left_fancyindex(
    ...
    if keep_right is not None or right_keys_all_unique:
        if is_inner_join:
            ...
            left_group_exists_in_right = full(left_keygroup.ncountgroup.shape, False, dtype=np.bool_)
...
def _create_merge_fancy_indices(
    ...
    def outer_join(
        ...
        right_only_group_mask = empty(right_keygroup.ncountgroup.shape, dtype=np.bool_)
...
class _AsOfMerge:
    ...
    def get_result(
        ...
        if direction == 'nearest':
            ...
            f_forward_closer = full(len(right_fancyindex_backward), False, dtype=np.bool_)
