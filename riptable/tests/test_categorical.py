...
# TODO: Move the tests below into one or more classes (just make sure the class is _not_
#       derived from unittest.TestCase, because pytest.mark.parametrize() doesn't support
#       being used on functions within a unittest.TestCase-derived class).
...
#PythonScalarOrWildcard = PythonScalar   # Union[PythonScalar, builtins.ellipsis]
PythonScalarOrWildcard = Union[PythonScalar, 'ellipsis']
...
def strict_tuple_comparison(
    ...
    x: Union[rt.Categorical, Tuple[PythonScalarOrWildcard, ...]],
    y: Union[rt.Categorical, Tuple[PythonScalarOrWildcard, ...]]
) -> np.ndarray:
    """
    ...
    if isinstance(x, rt.Categorical):
        if isinstance(y, rt.Categorical):
            ...
            # If tuple is single-element, unpack the element then call recursively
            # so we can re-use the more-efficient code path for 1D categorical vs. scalar comparison.
            if len(y) == 1:
                return strict_tuple_comparison(compare_func, x, y[0])

            # All components of the tuple are required to be scalars.
            for i in range(len(y)):
                if not np.isscalar(y[i]) and y[i] is not Ellipsis:
                    raise ValueError(f"The component of 'y' at index {i} is not a scalar or ellipsis.")
        ...
        # and raise the exception since it means we don't need to do it in each branch below.
            ...
            raise NotImplementedError("TODO: Implement support for 1-ary Categorical comparison vs. scalar")

        elif isinstance(y, np.ndarray):
            # Compare array against a 1-ary Cat (.issinglekey, .isenum).
            # TODO: Check array shape matches Categorical.
            raise NotImplementedError("TODO: Implement support for 1-ary Categorical comparison vs. array.")
        ...
        if isinstance(x, tuple):
            ...
            if len(x) != len(y_cat_dict):
                raise ValueError(f"Cannot compare a tuple of {len(x)} components against a Categorical on {len(y_cat_dict)} columns.")

            # If tuple is single-element, unpack the element then call recursively
            # so we can re-use the more-efficient code path for 1D categorical vs. scalar comparison.
            if len(x) == 1:
                return strict_tuple_comparison(compare_func, x[0], y)

            # All components of the tuple are required to be scalars.
            for i in range(len(x)):
                if not np.isscalar(x[i]) and x[i] is not Ellipsis:
                    raise ValueError(f"The component of 'x' at index {i} is not a scalar or ellipsis.")
...
class TestCategoricalCompare:
    ...
    def test_compare_multikey_cat_vs_multikey_cat(self, op_name: str, compatible_strs: bool):
        ...
        if not isinstance(reference_result, BaseException):  # anything other than exceptions
    ...
    def test_compare_multikey_cat_vs_tuple(self, op_name: str, tup: tuple):
        ...
        # Compare using the strict, Python-tuple-based reference implementation.
        compare_func = functools.partial(tuple_with_wildcard_compare_func, compare_func=getattr(operator, op_name))
        try:
